...
class VSSM_Fusion_Segmentation(nn.Module):
    """Extended FusionMamba model with segmentation capability
    
    This model can perform both image fusion and semantic segmentation.
    It extends the original VSSM_Fusion model by adding a segmentation head.
    """
    def __init__(self, patch_size=4, in_chans=1, num_classes=1000, depths=[2, 2, 9, 2], depths_decoder=[2, 9, 2, 2],
                 dims=[96, 192, 384, 768], dims_decoder=[768, 384, 192, 96], d_state=16, drop_rate=0.,
                 attn_drop_rate=0., drop_path_rate=0.1,
                 norm_layer=nn.LayerNorm, patch_norm=True,
                 use_checkpoint=False, num_seg_classes=9, **kwargs):
        super().__init__()
        
        # Initialize the base fusion model
        self.fusion_model = VSSM_Fusion(
            patch_size=patch_size, in_chans=in_chans, num_classes=num_classes,
            depths=depths, depths_decoder=depths_decoder,
            dims=dims, dims_decoder=dims_decoder, d_state=d_state,
            drop_rate=drop_rate, attn_drop_rate=attn_drop_rate, drop_path_rate=drop_path_rate,
            norm_layer=norm_layer, patch_norm=patch_norm, use_checkpoint=use_checkpoint, **kwargs
        )
        
        # Add segmentation head
        # The features before final_conv have shape [B, dims_decoder[-1]//4, H, W]
    ...
    def forward(self, x1, x2, return_fusion=True, return_segmentation=True):
        """Forward pass that can return both fusion and segmentation outputs
        
        Args:
            x1, x2: Input image pairs
            return_fusion: Whether to return fusion output
            return_segmentation: Whether to return segmentation output
            
        Returns:
            dict containing 'fusion' and/or 'segmentation' outputs
        """
        x_1 = x1
        x_2 = x2
        
        # Forward through the backbone (same as original model)
        x1, skip_list1 = self.fusion_model.forward_features_1(x1)
        x2, skip_list2 = self.fusion_model.forward_features_2(x2)
        
        x = x1 + x2
        skip_list = self.fusion_model.Fusion_network(skip_list1, skip_list2)
        
        # Forward through decoder layers
        x = self.fusion_model.forward_features_up(x, skip_list)
        
        # Get features before final convolution for segmentation
        features = self.fusion_model.final_up(x)  # [B, H, W, C]
        features_for_seg = features.permute(0, 3, 1, 2)  # [B, C, H, W]
        
        outputs = {}
        
        if return_segmentation:
            # Generate segmentation logits
            seg_logits = self.segmentation_head(features_for_seg)
            outputs['segmentation'] = seg_logits
            
        if return_fusion:
            # Generate fusion output (original path)
            fusion_output = self.fusion_model.final_conv(features_for_seg)
            fusion_output = fusion_output + x_1 + x_2 + x_1 + x_2  # Original fusion logic
            outputs['fusion'] = fusion_output
            
        return outputs
    
    def forward_fusion_only(self, x1, x2):
        """Forward pass for fusion only (matches original model interface)"""
        result = self.forward(x1, x2, return_fusion=True, return_segmentation=False)
        return result['fusion']
    
    def forward_segmentation_only(self, x1, x2):
        """Forward pass for segmentation only"""
        result = self.forward(x1, x2, return_fusion=False, return_segmentation=True)
