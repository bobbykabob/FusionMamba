...
def calculate_miou_per_sample(pred, target, num_classes=5):
    """Calculate IoU for each class present in the sample - only for classes that exist in PST900"""
...
def calculate_overall_miou(pred_np, target_np, num_classes=5):
    ...
    # Calculate IoU for each class (PST900 has classes 0-4)
    ious = []
...
def calculate_overall_macc(pred_np, target_np, num_classes=5):
    ...
    # Calculate accuracy for each class (PST900 has classes 0-4)
    accuracies = []
    for class_id in range(num_classes):
        pred_binary = (pred_np == class_id)
        target_binary = (target_np == class_id)
...
def calculate_macc_per_sample(pred, target, num_classes=5):
    """Calculate accuracy for each class present in the sample - only for classes that exist in PST900"""
    pred_np = pred.cpu().numpy().flatten()
    target_np = target.cpu().numpy().flatten()
...
def calculate_detailed_metrics(pred, target, num_classes=5):
    """Calculate detailed per-class metrics - only for classes that exist in PST900"""
    pred_np = pred.cpu().numpy().flatten()
    target_np = target.cpu().numpy().flatten()
    
    class_names = {
        0: 'Background', 1: 'Person', 2: 'Car', 3: 'Bicycle', 4: 'Motorcycle'
    }
...
def evaluate_pst900_segmentation():
    ...
    print("\n" + "="*70)
    print("PST900 Segmentation Evaluation Results (Improved Metrics):")
    print("="*70)
    ...
    with open(results_file, 'w') as f:
        f.write("PST900 Segmentation Evaluation Results (Improved Metrics):\n")
        f.write("="*60 + "\n")
